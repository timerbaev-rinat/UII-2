...
- [ ] Загрузка и привязка фото к карточке (при постановке на учёт) — есть поле `photo_path`, нужен upload-эндпоинт
...
- [ ] Загрузка/выгрузка файлов и фото, ограничения размера и типов
...
- [ ] HTTPS, CSP, настройка Cookie и заголовков безопасности
- [ ] Валидация входных данных, защита от загрузки вредоносных файлов (upload ещё не реализован)
- [x] Политика хранения паролей (bcrypt), смена пароля, JWT (access/refresh)
- [ ] Резервное копирование (pg_dump по расписанию), хранение вне сервера, процедура восстановления, тестовые восстановления
- [ ] Docker-сборка для продакшена, конфигурация через `.env` (Dockerfile есть, требуется доводка)
- [ ] Логирование ошибок, мониторинг, алерты

## Этап 12. Тестирование, документация и приёмка
**Цель:** проверка системы по чек-листу.

- [ ] Юнит-тесты (модели, сервисы, права доступа, нумерация)
- [ ] Интеграционные тесты API (ключевые сценарии: поступление→учёт→перемещение→инвентаризация→списание)
- [ ] Тесты генерации отчётов (проверка содержимого Excel/PDF/CSV/JSON)
- [ ] Пользовательская документация (роли, инструкции для смартфона и ПК)
- [ ] Сквозная проверка по чек-листу `chek list.md`
...
| 11 | Безопасность, бэкапы, деплой | 🔴 В работе (bcrypt/JWT готовы) |
| 12 | Тестирование, документация и приёмка | 🔴 Не начат |
...
1. **Тесты (Этап 12)** — юнит- и интеграционные тесты API ключевых сценариев
   (поступление → учёт → перемещение → инвентаризация → списание). Сейчас их нет вовсе.
2. **Загрузка файлов и фото (Этап 4/7)** — модуль `attachments`:
   upload/выгрузка фото карточек и файлов документов, ограничение размера/типов.
...
7. **Безопасность и деплой (Этап 11)** — HTTPS/CSP/заголовки, защита upload,
   pg_dump-бэкапы по расписанию, доводка продакшен-Docker, логирование/мониторинг.
